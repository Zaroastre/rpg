...
class AttackStategy:
    @abstractmethod
    def execute(self, target: AbstractCharacter) -> None:
        raise NotImplementedError()
...
class Fighter:
    def __init__(self, breed_type: BreedType, character_class: Class, level: Level, weapon: Weapon) -> None:
...
class AttackStategyChooser:
    def __init__(self, breed_type: BreedType, character_class: Class, level: Level, weapon: Weapon) -> None:
        self.__attacker: Fighter = Fighter(breed_type, character_class, level, weapon)

    def __choose_strategy_for_mage(self, target: Fighter) -> AttackStategy:
        ...
        if (len(damages_spells) > 0 or len(infect_spells) > 0):
            ...
            if (self.__attacker.weapon is not None):
                strategy = WeaponAttackStrategy(self.__attacker.weapon)
        return strategy
    def __choose_strategy_for_priest(self, target: Fighter) -> AttackStategy:
        strategy: AttackStategy = UnarmedAttackStategy()
        return strategy
    def __choose_strategy_for_demonist(self, target: Fighter) -> AttackStategy:
        strategy: AttackStategy = UnarmedAttackStategy()
        return strategy
    def __choose_strategy_for_shaman(self, target: Fighter) -> AttackStategy:
        strategy: AttackStategy = UnarmedAttackStategy()
        return strategy
    def __choose_strategy_for_druid(self, target: Fighter) -> AttackStategy:
        strategy: AttackStategy = UnarmedAttackStategy()
        return strategy
    def __choose_strategy_for_paladin(self, target: Fighter) -> AttackStategy:
        strategy: AttackStategy = UnarmedAttackStategy()
        return strategy
    def __choose_strategy_for_warrior(self, target: Fighter) -> AttackStategy:
        strategy: AttackStategy = UnarmedAttackStategy()
        return strategy
    def __choose_strategy_for_monk(self, target: Fighter) -> AttackStategy:
        strategy: AttackStategy = UnarmedAttackStategy()
        return strategy
    def __choose_strategy_for_hunter(self, target: Fighter) -> AttackStategy:
        strategy: AttackStategy = UnarmedAttackStategy()
        return strategy
    def __choose_strategy_for_rogue(self, target: Fighter) -> AttackStategy:
        strategy: AttackStategy = UnarmedAttackStategy()
        return strategy
    def __choose_strategy_for_death_knight(self, target: Fighter) -> AttackStategy:
        strategy: AttackStategy = UnarmedAttackStategy()
        return strategy
    def __choose_strategy_for_demon_hunter(self, target: Fighter) -> AttackStategy:
        strategy: AttackStategy = UnarmedAttackStategy()
        return strategy

    def choose_best_attack_strategy(self, breed_type: BreedType, character_class: Class, level: Level, weapon: Weapon) -> AttackStategy:
        target: Fighter = Fighter(breed_type, character_class, level, weapon)
        strategy: AttackStategy = UnarmedAttackStategy()
